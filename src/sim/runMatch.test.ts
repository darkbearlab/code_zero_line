...
describe('headless simulator', () => {
  it('produces identical outcomes for identical seeds (determinism)', () => {
    ...
    const a = simulateMatch(s1, greedyController, greedyController);
    const b = simulateMatch(s2, greedyController, greedyController);
  ...
  it('terminates within maxCommands', () => {
    ...
    const out = simulateMatch(s, greedyController, greedyController, {
      maxCommands: 3000,
    });
  ...
  it('greedy vs greedy produces a valid winner / draw on each match', () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      outcomes.push(simulateMatch(s, greedyController, greedyController));
    }
